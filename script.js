...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const statusText = document.querySelector('.form-status'); // <p class="form-status">
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (statusText) {
          statusText.textContent = "Une erreur s'est produite. Merci de réessayer.";
        }
      }
    } catch (error) {
      if (statusText) {
        statusText.textContent = "Impossible d'envoyer le formulaire. Vérifiez votre connexion.";
      }
    }
  });
